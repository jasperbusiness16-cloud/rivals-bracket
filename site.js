...
donationsRef.on("value", (snapshot) => {
  ...
  updateDonationTotals(latestDonations);
});
...
function updateDonationTotals(donations) {
  const startingPrizePool = numberFromMoney(latestSiteData.startingPrizePool || "$0");
  const donationTotal = donations.reduce((sum, d) => sum + d.amount, 0);
  const currentPrizePool = startingPrizePool + donationTotal;

  const goalAmount = numberFromMoney(latestSiteData.donationGoal || "$250");
  const goalPercent = goalAmount > 0
    ? Math.min((currentPrizePool / goalAmount) * 100, 100)
    : 0;

  document.querySelectorAll("[data-starting-prize-pool]").forEach(el => {
    el.innerText = money(startingPrizePool);
  });

  document.querySelectorAll("[data-community-donations]").forEach(el => {
    el.innerText = `+${money(donationTotal)}`;
  });

  document.querySelectorAll("[data-prize-pool]").forEach(el => {
    el.innerText = money(currentPrizePool);
  });
  ...
  document.querySelectorAll("[data-goal-target]").forEach(el => {
    el.innerText = `${latestSiteData.donationGoal || "$250"} Goal`;
  });

  document.querySelectorAll("[data-goal-percent]").forEach(el => {
    el.innerText =
      `${money(currentPrizePool)} / ${latestSiteData.donationGoal || "$250"} (${Math.round(goalPercent)}%)`;
  });
